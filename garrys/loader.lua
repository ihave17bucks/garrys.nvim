...
-- the main problem is that vim.opt.rtp:prepend() updates Neovim's rtp
-- but does NOT update Lua's package.path. Plugins with nested
-- require() calls (like nvim-notify) can't find their submodules
-- the fix is manually sync package.path every time we inject a plugin :sob:
...
-- and also sync package.cpath for compiled .so modules (fzf-native etc.)
...
function M.inject(plugin)
	-- run the init before anything else (lazy.nvim compat) because most people will migrate from it
	if plugin.init then
	...
	-- sync Lua package.path so nested requires work
	...
	-- run config if explicitly provided
	-- dont fucking auto-require(plugin.name)
	-- module names rarely match repo names
	if plugin.on then
