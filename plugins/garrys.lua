	...
	for _, plugin in pairs(plugins) do
		...
		if not u.is_installed(plugin.path) then
			...
			if has_lua then
				...
				if not ok then
					table.insert(issues, "require() failed")
				end
			end
		end
	...
	vim.schedule(function()
		require("garrys.util").info(
			"next steps:\n"
				...
				.. "  3. open Neovim then it will find the missing plugins install automatically"
		)
	end)
