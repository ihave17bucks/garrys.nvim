...
-- checks if a plugin is installed on disk
function M.is_installed(path)
	return vim.loop.fs_stat(path) ~= nil
end

-- this consistent plugin install path
function M.plugin_path(base, name)
	return base .. "/" .. name
end

-- scans dirs and return all subdirs names
...
-- runs a build hook after the install
...
-- sorts plugins respecting dependencies
