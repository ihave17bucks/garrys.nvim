...
-- the main prob is tha vim.opt.rtp:prepend() updates nvim's rtp
-- but does NOT update Lua's package.path. plugins with nested
-- require() calls (like nvim-notify) can't find their submodules.
-- the fix is manually sync package.path every time we inject a plugin.
...
function M.inject(plugin)
	...
	-- never auto-require(plugin.name)
