...
function M.setup(specs, opts)
  ...
  if not ok then
    -- WARNS but not for neovim configs
...
function M.plug(spec)
  local ok, err = pcall(function()
    ...
    local plugin = M._normalize(spec)
    if plugin then M._plugins[plugin.name] = plugin end
  end)
  if not ok then
...
function M._normalize(spec)
  ...
  -- handles lazy.nvim url = "..." specs
  if not source and spec.url then
    source = spec.url:match("([^/]+/[^/%.]+)%.git$") or spec.url
  end

  -- handles local dir = "..." specs just add to rtp, no install needed
  if not source and spec.dir then
    local ok = pcall(function()
      if vim.loop.fs_stat(spec.dir) then
        ...
        -- runs the init if provided (lazy.nvim compat)
  ...
  if spec.cond ~= nil then
    ...
    if not cond_ok or not result then return nil end
  end
  ...
  -- accept dependencies (lazy.nvim), depends, AND dep (garrys short form) because yeah..i guess
  ...
  for _, d in ipairs(raw_deps) do
    ...
    elseif type(d) == "table" and d[1] then
      ...
      -- also registers sub-spec as its own plugin
  ...
  return {
    ...
    -- lazy.nvim fields are quietly accepted but not used that much
    -- priority, dev, version, module no errors, no warnings, etc this is kinda boring
    _loaded = false,
  }
end
            ...
            git.clone(plugin.url, plugin.path, function(clone_ok, clone_err)
              ...
              vim.schedule(function()
                if clone_ok then
                  ...
                  -- safety injects and warns on for failure and it will never crash
                  local inject_ok, inject_err = pcall(loader.inject, plugin)
                  if not inject_ok then
                    vim.notify("[garrys] inject failed for " .. plugin.name .. ": " .. tostring(inject_err),
                      vim.log.levels.WARN)
                  end
                  if plugin.make then
                    local build_ok, build_err = pcall(u.run_build, plugin)
                    if not build_ok then
                      vim.notify("[garrys] build failed for " .. plugin.name .. ": " .. tostring(build_err),
                        vim.log.levels.WARN)
                    end
                  end
                else
                  ...
                  git.clone(plugin.url, plugin.path, function(retry_ok, retry_err)
                    vim.schedule(function()
                      if retry_ok then
                        ...
                        ui.set_status(plugin.name, "✘ " .. (retry_err or clone_err or "failed"):gsub("\n", " "))
                        vim.notify("[garrys] " .. plugin.name .. " failed after retry — skipping", vim.log.levels.WARN)
                      end
                    end)
                  end)
                end
                if done == #missing then ui.finish() else dispatch() end
              end)
            end)
          end
        end
