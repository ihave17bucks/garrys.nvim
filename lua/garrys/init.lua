              ...
              vim.schedule(function()
                if clone_ok then
                  ...
                  if not inject_ok then
                    vim.notify("[garrys] inject failed for " .. plugin.name .. ": " .. tostring(inject_err), vim.log.levels.WARN)
                  end
                  if plugin.make then
                    local build_ok, build_err = pcall(u.run_build, plugin)
                    if not build_ok then
                      vim.notify("[garrys] build failed for " .. plugin.name .. ": " .. tostring(build_err), vim.log.levels.WARN)
                    end
                  end
                else
